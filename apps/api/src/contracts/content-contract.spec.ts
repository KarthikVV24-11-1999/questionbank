...
} from '../contexts/content/application/review/policies.js';
import { REJECTION_REASONS as DOMAIN_REJECTION_REASONS } from '../contexts/content/domain/review/rejection-taxonomy.js';
import { REJECTION_REASONS as CONTRACTS_REJECTION_REASONS } from '@questionbank/contracts/review-taxonomy';
...
const HANDLER_POLICIES = {
  ...
  ClaimNextForReview: CLAIM_NEXT_FOR_REVIEW_POLICY,
  ReleaseAssignment: RELEASE_ASSIGNMENT_POLICY,
  ReassignReview: REASSIGN_REVIEW_POLICY,
  ExtendLease: EXTEND_LEASE_POLICY,
  ApproveWithEdits: APPROVE_WITH_EDITS_POLICY,
  GetDuplicateCandidates: GET_DUPLICATE_CANDIDATES_POLICY,
  GetQueueHealth: GET_QUEUE_HEALTH_POLICY,
  GetReviewerThroughput: GET_REVIEWER_THROUGHPUT_POLICY,
} as const;
...
describe('ADR-0009 condition 1 — the authoring route list is enumerated and closed', () => {
  ...
  /**
   * M4-36's own planted violation: DEC-M4-12 exists specifically so a
   * key-bearing review route never needs its own prefix — it mounts inside
   * `/v1/authoring/**` instead, precisely to keep this check meaningful. A
   * route planted at `/v1/review/**` (the prefix DEC-M4-12 rejected) proves
   * both directions of the enumeration check would refuse it, without
   * risking the real document ever taking that shape.
   */
  it('is red on a planted /v1/review/** route — the prefix DEC-M4-12 exists to avoid', () => {
    const plantedRoute = '/v1/review/assignments';
    const plantedPaths = { ...document.paths, [plantedRoute]: { post: {} } };

    // Direction 1: "lists exactly the /v1/authoring paths the document
    // declares" — a planted route the enumeration wrongly omitted fails
    // when it is added to `paths` but not to the enumeration.
    const declared = Object.keys(plantedPaths)
      .filter((path) => path.startsWith('/v1/authoring/'))
      .sort();
    expect(declared).not.toContain(plantedRoute);
    expect([...authoringRoutes].sort()).not.toEqual(
      Object.keys(plantedPaths)
        .filter((path) => path.startsWith('/v1/authoring/') || path === plantedRoute)
        .sort(),
    );

    // Direction 2: "is a list, not a prefix rule" — were the planted route
    // added to the enumeration by mistake, this assertion (run over every
    // real entry today) would refuse it on sight.
    expect(plantedRoute.startsWith('/v1/authoring/')).toBe(false);
  });
});
...
describe('M4-36 — no reviewer justification text on a review list endpoint', () => {
  it('finds no justification field on the queue-health, throughput or duplicate-candidates responses', () => {
    const listLike = ['QueueHealthResult', 'ReviewerThroughputResult', 'DuplicateCandidatesResult'];
    for (const schemaName of listLike) {
      const fields = reachableFieldNames(schemaName);
      expect(fields, schemaName).not.toContain('justification');
    }
  });

  it('is not vacuous — the same field is present where a single decision is recorded', () => {
    expect(reachableFieldNames('AuthoringReviewDecisionRequest')).toContain('justification');
  });
});
...
/**
 * M4-39. `REJECTION_REASONS` cannot be generated by `generate-zod.mjs` — it
 * has never been an HTTP request or response body, and the generator walks
 * `components.schemas` only. It is instead hand-kept in two places:
...
describe('M4-39 — the rejection taxonomy mirror agrees with its domain declaration', () => {
  it('is byte-for-byte the same table, in the same order', () => {
    expect(CONTRACTS_REJECTION_REASONS).toEqual(DOMAIN_REJECTION_REASONS);
  });

  it('checked a table with something in it — not vacuously equal empty arrays', () => {
    expect(DOMAIN_REJECTION_REASONS.length).toBeGreaterThan(5);
  });
});
