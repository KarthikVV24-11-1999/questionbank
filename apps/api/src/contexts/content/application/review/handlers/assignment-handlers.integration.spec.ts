...
function deps(): AssignmentDependencies {
  return { assignments, items, fingerprints, reviewPolicy: POLICY, clock, audit };
}
...
/**
 * A fully-formed, valid item, submitted for review — via `ItemRepository`,
 * not raw SQL, so `findById` reconstitutes it and `ClaimNextForReviewHandler`'s
 * fingerprint step (M4-32) has a real `ContentBody` to hash. `seedInReviewItemVersion`
 * above is deliberately minimal (raw rows only) for the M4-18/M4-27 tests
 * that never touch content itself; this is the M4-32 tests' own fixture.
 */
async function fullInReviewItem(subject = `full-${freshUuid()}`): Promise<{ itemId: string; itemVersionId: string }> {
    ...
    createItemVersion(
      {
        ...
        authoredBy: { kind: 'human', id: AUTHOR_ID, roleContext: ['author'] },
  ...
  const submitted = expectValue(transitionItem(created, { transition: 'submit_for_review', stateEnteredAt: new Date().toISOString() }));
...
describe('ClaimNextForReviewHandler computes the claimed item’s fingerprint (M4-32)', () => {
  it('computes and persists it when missing, so the reviewer never sees not_evaluated for the item in front of them', async () => {
    const subject = `fp-${freshUuid()}`;
    const { itemVersionId } = await fullInReviewItem(subject);

    const before = expectValue(await fingerprints.findByItemVersionId(itemVersionId));
    ...
    const after = expectValue(await fingerprints.findByItemVersionId(itemVersionId));
    expect(after).toMatchObject({ itemVersionId, subject });
    expect(after!.exactHash).toMatch(/^[0-9a-f]{64}$/u);
    expect(after!.skeletonHash).toMatch(/^[0-9a-f]{64}$/u);
  });

  it('does not overwrite an existing fingerprint on a later claim', async () => {
    const subject = `fp-stable-${freshUuid()}`;
    const { itemVersionId } = await fullInReviewItem(subject);
    const handler = new ClaimNextForReviewHandler(deps());
    expectValue(await handler.handle({ subject }, as(reviewer({ roleContext: ['reviewer', `subject:${subject}`] }))));
    const first = expectValue(await fingerprints.findByItemVersionId(itemVersionId));

    // Release and re-claim (the version's own fingerprint never changes — it
    // is immutable content, M4-04) to prove the "if missing" check actually
    // skips recomputation rather than upserting on every claim.
    const reclaimSubject = `fp-stable-reclaim-${freshUuid()}`;
    const { itemVersionId: reclaimVersionId } = await fullInReviewItem(reclaimSubject);
    expectValue(
      await handler.handle({ subject: reclaimSubject }, as(reviewer({ roleContext: ['reviewer', `subject:${reclaimSubject}`] }))),
    );
    const reclaimed = expectValue(await fingerprints.findByItemVersionId(reclaimVersionId));
    expect(reclaimed!.computedAt).toBeDefined();
    // The first item's own record is untouched by claiming a second, unrelated one.
    const stillFirst = expectValue(await fingerprints.findByItemVersionId(itemVersionId));
    expect(stillFirst).toEqual(first);
  });

  it('never fails the claim when the fingerprint step cannot resolve the item — the minimal M4-18/M4-27 fixture has no full aggregate', async () => {
    const subject = `fp-minimal-${freshUuid()}`;
    const { itemVersionId } = await seedInReviewItemVersion({ subject });
    const handler = new ClaimNextForReviewHandler(deps());
    const claimed = expectValue(await handler.handle({ subject }, as(reviewer({ roleContext: ['reviewer', `subject:${subject}`] }))));
    expect(claimed.itemVersionId).toBe(itemVersionId);
    // No crash, and honestly no fingerprint either — findById cannot
    // reconstitute a raw-seeded row with no options/provenance.
