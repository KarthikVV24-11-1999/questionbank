...
export class PostgresReviewEscalationRepository implements ReviewEscalationRepository {
  ...
  async findNotifiedAt(itemVersionIds: readonly string[]): Promise<Result<ReadonlyMap<string, string>, RepositoryError>> {
    if (itemVersionIds.length === 0) return ok(new Map());
    try {
      const found = await this.#pool.query<{ item_version_id: string; escalated_at: Date }>(
        `SELECT item_version_id, escalated_at FROM content.review_escalation WHERE item_version_id = ANY($1::uuid[])`,
        [itemVersionIds],
      );
      return ok(new Map(found.rows.map((row) => [row.item_version_id, row.escalated_at.toISOString()])));
    } catch (error) {
