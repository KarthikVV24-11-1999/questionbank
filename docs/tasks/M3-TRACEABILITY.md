...
| **W3** | M3-26 — subject-scoped authoring is enforced | A Chemistry author reaching for Physics is refused. But **the subject is declared on the command and is not cross-checked against the content**: nothing records the subject of a passage, and `curriculum/public/` exposes no concept→subject-domain lookup. An in-subject author mistagging their own work is not caught. **M3-26's own scope was stimulus/solution/media/import only — it never covered items**, and `CreateItemDraft` had no subject enforcement at all until M4-14a closed it (correction, 2026-08-20, full account in `M3-CLOSEOUT.md`'s debt register). | **D23** |
