...
| ↳ **Correction, 2026-08-20 (M4-14a).** This was true for stimulus, solution, media and import — it was never true for items: `CreateItemDraft` had no subject field and `CreateItemDraftHandler` never called `authorizeSubjectScope` at all, so FR-TCH-01 rule 1 was unenforced on the primary authoring path (found and proven with a before/after test in M4-14a, not merely tidied). For items, D23 is now **partially closed**: the subject is *resolved* from a single-scoped author's own scope rather than declared, so an in-scope author can no longer misdeclare their own work. The half D23 always named stays open, unchanged: nothing cross-checks a resolved or declared subject against the content itself, which needs a concept→subject-domain lookup Curriculum still does not expose. Stimulus, solution and media still declare-and-check; aligning them to resolution is worth doing when one is next touched, not done as part of this correction. |
